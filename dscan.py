...
class Crawler(File_Manager):
    ...
    CONDITIONS = {
                ...
                "is_current_host"     : ( lambda self, link: False if tldextract.extract(link).domain != tldextract.extract(self.host).domain else True ),
    ...
    def __init__(self, host, payload_path):
    ...
    async def request(self, page, req_sema, client):
        ...
        async with req_sema:
            ...
            try:
                ...
                await self.write_to_file(row=[page.method, page.url,page.first_load_time,page.second_load_time,page.cache_control,page.expires,page.etag,page.last_modified])
    ...
    def __validate_link__(self, obj, parent = None):
        ...
        if obj.name == 'form':
            ...
            page.payload = { i.get('name') : random.sample(self.payloads,1) for i in obj.find_all(lambda tag:
                                                                        tag.name == "input" and
                                                                        tag.attrs.get('type') not in ('submit','hidden')) }
        else:
            page.url = urlparse(obj.get('href'))._replace(query=None).geturl()
        if page.url:
            page.url = self.CONDITIONS['part_of_link'](self, page.url)
            if self.CONDITIONS['is_not_visited'](self, (page.url,page.method)) and self.CONDITIONS['is_current_host'](self, page.url) and self.CONDITIONS['not_in_ext'](self, page.url) and self.CONDITIONS['is_url'](self, page.url):
...
if __name__ == '__main__':
    ...
    try:
        # Get args
        parser = argparse.ArgumentParser(prog='ddos_scanner',usage='%(prog)s [options] hosts separated by spaces [payload]')
